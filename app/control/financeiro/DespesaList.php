...
class DespesaList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $criteria_status_lancamento_id = new TCriteria();
        ...
        $status_lancamento_id = new TDBCombo('status_lancamento_id', 'bd_sgp', 'StatusLancamento', 'id', '{descricao}','id asc' , $criteria_status_lancamento_id );
        ...
        $filterVar = TipoLancamento::DESPESA;
        ...
        $column_pagamento_transformed = new TDataGridColumn('pagamento', "Pagamento", 'left');
        ...
        $action_onEdit = new TDataGridAction(array('DespesaForm', 'onEdit'));
        $action_onEdit->setUseButton(false);
        $action_onEdit->setButtonClass('btn btn-default btn-sm');
        $action_onEdit->setLabel("Editar");
        $action_onEdit->setImage('far:edit #478fca');
        $action_onEdit->setField(self::$primaryKey);
        $action_onEdit->setDisplayCondition('DespesaList::onExibeEditar');

        $this->datagrid->addAction($action_onEdit);

        $action_onDelete = new TDataGridAction(array('DespesaList', 'onDelete'));
        $action_onDelete->setUseButton(false);
        $action_onDelete->setButtonClass('btn btn-default btn-sm');
        $action_onDelete->setLabel("Excluir");
        $action_onDelete->setImage('fas:trash-alt #dd5a43');
        $action_onDelete->setField(self::$primaryKey);
        $action_onDelete->setDisplayCondition('DespesaList::onExibeDeletar');
        ...
        $panel = new TPanelGroup("Listagem de despesas");
        ...
        $button_despesa = new TButton('button_button_despesa');
        $button_despesa->setAction(new TAction(['DespesaForm', 'onShow']), "Despesa");
        $button_despesa->addStyleClass('btn-default');
        $button_despesa->setImage('fas:plus #69aa46');

        $this->datagrid_form->addField($button_despesa);

        $button_pagar_em_lote = new TButton('button_button_pagar_em_lote');
        $button_pagar_em_lote->setAction(new TAction(['DespesaList', 'onPagarAll']), "Pagar em Lote");
        $button_pagar_em_lote->addStyleClass('btn-default');
        $button_pagar_em_lote->setImage('fas:money-bill-alt #4CAF50');

        $this->datagrid_form->addField($button_pagar_em_lote);

        $btnShowCurtainFilters = new TButton('button_btnShowCurtainFilters');
        $btnShowCurtainFilters->setAction(new TAction(['DespesaList', 'onShowCurtainFilters']), "Filtrar");
        ...
        $button_deletar->setAction(new TAction(['DespesaList', 'onDeleteBatchArray']), "Deletar");
        ...
        $button_parcelas->setAction(new TAction(['DespesaFormParcela', 'onShow']), "Parcelas");
        ...
        $head_left_actions->add($button_despesa);
        $head_left_actions->add($button_pagar_em_lote);
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Financeiro","Despesas"]));
        }
    ...
    public function onPagarAll($param = null) 
    {
        try 
        {
            ...
            if(array_key_exists('builder_datagrid_check', $param)){
               ...
               AdiantiCoreApplication::loadPage('ProcessarPagamentoForm', 'onShow', $param);
            }else{
    ...
    public static function onShowCurtainFilters($param = null) 
    {
        try 
        {
            ...
            $page->setProperty('page-name', 'DespesaListSearch');
            $page->setProperty('page_name', 'DespesaListSearch');
            ...
            $style = new TStyle('right-panel > .container-part[page-name=DespesaListSearch]');
    ...
    public function onShow($param = null)
    {
        ...
        $dados_sessao = TSession::getValue('DespesaListbuilder_datagrid_check');

        if(!empty($dados_sessao)){
            // Código gerado pelo snippet: "Exibir mensagem"
            new TMessage('info', "Existem dados que já estão sendo utilizados em outra aba do seu navegador! A tela de despesas não pode ser carregada em outra aba!");

            $pageParam = []; // ex.: = ['key' => 10]

            TApplication::loadPage('PlanoList', 'onShow', $pageParam);
